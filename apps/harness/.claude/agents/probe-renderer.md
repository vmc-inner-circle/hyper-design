...
- 컴포넌트 루트 `data-component="<screens.md 구성 열 이름 그대로>"`(default 프레임 최상위 집합 = 구성표). 탭 가능한 요소 전부 `data-tap`, 탭바·세그먼트처럼 칸이 붙은 묶음의 부모는 `data-tap-group`(tap.gap 면제). primary 버튼 `data-primary`(default 프레임에 정확히 1개, 예외 화면은 `<section>`에 `data-primary-exempt="<design-rules §C 키>"` — 예: `web.primary`, 빈 값은 실패). 고정 바 `data-fixed="cta|tabbar"`, 홈 인디케이터 영역 `data-safe-area`(고정 바 안 또는 바로 아래 형제). 스크롤 본문 `data-scroll`(고정 바가 있는 프레임엔 필수). 말줄임 `data-truncate="1|2|3"`. 아이콘은 `<i data-icon="<lucide 이름>"><svg stroke="currentColor">`(래퍼 없는 `<svg>` 금지, icons.md 허용 목록만).
